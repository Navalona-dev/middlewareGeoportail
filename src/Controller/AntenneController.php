...
use App\Service\AntenneService;
...
class AntenneController extends AbstractController
{
    ...
    private $pathImageAntenne = null;
    private $pathPublic = null;
    private $pathForNamePhotoAntenne = null;
    private $kernelInterface;
    private $directoryCopy = null;
    private const nameRepertoireImage = 'ar_antenne_relai/t_ar_01_infrastructure/';
    ...
    public function __construct(ParameterBagInterface $params, KernelInterface  $kernelInterface) {
        ...
        $this->pathImageAntenne = $params->get('pathImageAntenne');
        $this->pathPublic = $params->get('pathPublic');
        $this->pathForNamePhotoAntenne = $params->get('pathForNamePhotoAntenne');
    ...
    /**
     * @Route("/api/antenne/getphoto/{id}", name="infra_antenne_photo", methods={"GET"})
     */
    public function getPhotosByInfra($id, Request $request, AntenneService $antenneService)
    {
        $infoPhotosInfra = [];
        $response = new Response();
        if (isset($id) && !empty($id)) {
            $infoPhotosInfra = $antenneService->getPhotoInfraInfo($id);
    ...
    /**
     * @Route("/api/antenne/deletephoto", name="antenne_delete_photo", methods={"POST"})
     */
    public function deletePhoto(Request $request, AntenneService $antenneService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $infoPhotosInfra = $antenneService->getPhotoInfraInfo($idInfra);
                ...
                $directory = $this->pathImageAntenne . $indexPhoto."/";
                ...
                if (array_key_exists($indexPhoto, $oldPhotosInfra)) {
                    ...
                    if (isset($setUpdate) && !empty($setUpdate)) {
                        $idInfra = $antenneService->addInfrastructurePhoto($idInfra, $setUpdate);
                    }
                   
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Photo antenne route deleted_successfull"
                    ]));
                } else {
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Pas de photo antenne route supprimer"
                    ]));
                }
    ...
    /**
     * @Route("/api/antenne/updatephoto", name="antenne_update_photo", methods={"POST"})
     */
    public function updatePhoto(Request $request, AntenneService $antenneService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if(!is_dir($this->pathImageAntenne)) {
                mkdir($this->pathImageAntenne, 0777, true);
            }
          
            $directory1 = $this->pathImageAntenne . "photo1/";
            ...
            if (null != $uploadedFile1 && "null" != $uploadedFile1 && "undefined" != $uploadedFile1) {
                ...
                $data['photo1'] = $this->pathForNamePhotoAntenne."photo1/" .$nomPhoto1;
            ...
            $directory2 = $this->pathImageAntenne . "photo2/";
            ...
            if (null != $uploadedFile2 && "null" != $uploadedFile2 && "undefined" != $uploadedFile2) {
                ...
                $data['photo2'] = $this->pathForNamePhotoAntenne."photo2/" .$nomPhoto2;
            ...
            $directory3 = $this->pathImageAntenne . "photo3/";
            ...
            if (null != $uploadedFile3 && "null" != $uploadedFile3 && "undefined" != $uploadedFile3) {
                ...
                $data['photo3'] = $this->pathForNamePhotoAntenne."photo3/" .$nomPhoto3;
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "Photo antenne route updated_successfull"
            ]));
    ...
    /**
     * @Route("/api/antenne/add", name="antenne_add", methods={"POST"})
     */
    public function create(Request $request, AntenneService $antenneService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            // Data collecte
            $data['etatFondationPylone'] = $request->get('etatFondationPylone');
            $data['etatPylone'] = $request->get('etatPylone');
            $data['etatAntenne'] = $request->get('etatAntenne');
            $data['etatMateriels'] = $request->get('etatMateriels');
            ...
            if ($idInfra != false) {
                ...
                //$idEtat = $antenneService->addInfrastructureRouteEtat($idInfra, $data);

                $idEtat = $antenneService->addInfrastructureSituation($idInfra, $data);

                $idDataCollected = $antenneService->addInfrastructureDonneCollecte($idInfra, $data);

                /*$idStructure = $antenneService->addInfrastructureRouteStructure($idInfra, $data);

                $idAccotement = $antenneService->addInfrastructureRouteAccotement($idInfra, $data);

                $idFosse = $antenneService->addInfrastructureRouteFosse($idInfra, $data);*/
                ...
                $idFoncier = $antenneService->addInfrastructureRouteFoncier($idInfra, $data);*/
                ...
                if (null != $request->get('hasTravaux') && ($request->get('hasTravaux') == true || $request->get('hasTravaux') == "true") && "false" != $request->get('hasTravaux')) {
                    ...
                    $idTravaux = $antenneService->addInfrastructureTravaux($idInfra, $data);
                }
                ...
                $idFourniture = $antenneService->addInfrastructureRouteFourniture($idInfra, $data);*/
                // Etudes
                ...
                // $data['precisionConsistanceContratEtude'] = $request->get('precisionConsistanceContratEtude');
                    ...
                    $idEtude = $antenneService->addInfrastructureEtudes($idInfra, $data);
                }
                
                /**
                 * End Administrative data
                */
                //$idDonneAnnexe = $antenneService->addInfrastructureEducationDonneAnnexe($idInfra, $data);
            }

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "antenne route created_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'situation');
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'data');
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'surface');
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'structure');
            
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'accotement');
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'fosse');
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'foncier');
           
            $antenneService->cleanTablesByIdInfrastructure($idInfra, 'fourniture');*/
    ...
    /**
     * @Route("/api/infra/antenne/liste", name="antenne_list", methods={"GET"})
     */
    public function listeantenne(Request $request, AntenneService $antenneService)
    {    
        $response = new Response();
        
        try {

            $routes = $antenneService->getAllInfrastructures();

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "antenne route list_successfull",
                'pathImage' => $this->pathImage,
                'data' => $routes
            ]));
    ...
    /**
     * @Route("/api/infra/antenne/liste/minifie", name="antenne_list_minifie", methods={"GET"})
     */
    public function listeantenneMinifie(Request $request, AntenneService $antenneService)
    {    
        $response = new Response();
        
        try {

            $routes = $antenneService->getAllInfrastructuresMinifie();
    ...
    /**
     * @Route("/api/infra/antenne/info", name="antenne_info", methods={"POST"})
     */
    public function getOneInfraInfo(Request $request, AntenneService $antenneService)
    {    
        $response = new Response();
        ...
        try {
            ...
            $routes = $antenneService->getOneInfraInfo(intval($infraId));

            $routesInfrastructure = $antenneService->getAllyRouteInfoMinifie();
            $infoRoutes = [];
    ...
    /**
     * @Route("/api/antenne/update", name="antenne_update", methods={"POST"})
     */
    public function update(Request $request, AntenneService $antenneService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                if (array_key_exists('infrastructure', $data) && count($data['infrastructure']) > 0) {
                    ...
                    $allCategories = $antenneService->getAllCategorieInfra();
                    ...
                    $idInfra = $antenneService->updateInfrastructure($idInfra, $updateColonneInfra);
                    }
                }
                // Situation
                ...
                if (array_key_exists('situations', $data) && count($data['situations']) > 0) {
                    ...
                    if ($idSituation == 0) {
                        $idSituation = $antenneService->addInfoInTableByInfrastructure('t_ar_03_situation', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtat) && !empty($updateColonneEtat)) {
                        $idSituation = $antenneService->updateInfrastructureTables('t_ar_03_situation', $idSituation, $updateColonneEtat);
                        }
                    } 
                ...
                if (array_key_exists('data_collecte', $data) && count($data['data_collecte']) > 0) {
                    ...
                    if ($idData == 0) {
                        $idData = $antenneService->addInfoInTableByInfrastructure('t_ar_06_donnees_collectees', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneData) && !empty($updateColonneData)) {
                        $idData = $antenneService->updateInfrastructureTables('t_ar_06_donnees_collectees', $idData, $updateColonneData);
                        }
                    }
                }
                // Travaux
                ...
                if (array_key_exists('travaux', $data) && count($data['travaux']) > 0) {
                    ...
                    if ($idTravaux == 0) {
                        $idTravaux = $antenneService->addInfoInTableByInfrastructure('t_bc_05_travaux', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneTravaux) && !empty($updateColonneTravaux)) {
                        $idTravaux = $antenneService->updateInfrastructureTables('t_bc_05_travaux', $idTravaux, $updateColonneTravaux);
                        }
                    }
                }
                ...
                if (array_key_exists('etudes', $data) && count($data['etudes']) > 0) {
                    ...
                    if ($idEtudes == 0) {
                        $idEtudes = $antenneService->addInfoInTableByInfrastructure('t_bc_07_etudes', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtudes) && !empty($updateColonneEtudes)) {
                        $idEtudes = $antenneService->updateInfrastructureTables('t_bc_07_etudes', $idEtudes, $updateColonneEtudes);
                        }
                    }
                }
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "antenne update_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            //$antenneService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            //$antenneService->cleanTablesByIdInfrastructure($idInfra, 'etat');
            //$antenneService->cleanTablesByIdInfrastructure($idInfra, 'data');
            //$antenneService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            //$antenneService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
