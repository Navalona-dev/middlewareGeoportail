...
use App\Service\AssainissementService;
...
class AssainissementController extends AbstractController
{
    ...
    private $pathImageAssainissement = null;
    private $pathPublic = null;
    private $pathForNamePhotoAssainissement = null;
    private $kernelInterface;
    private $directoryCopy = null;
    private const nameRepertoireImage = 'as_assainissement/t_as_01_infrastructure/';
    ...
    public function __construct(ParameterBagInterface $params, KernelInterface  $kernelInterface) {
        ...
        $this->pathImageAssainissement = $params->get('pathImageAssainissement');
        $this->pathPublic = $params->get('pathPublic');
        $this->pathForNamePhotoAssainissement = $params->get('pathForNamePhotoAssainissement');
    ...
    /**
     * @Route("/api/assainissement/getphoto/{id}", name="infra_assainissement_photo", methods={"GET"})
     */
    public function getPhotosByInfra($id, Request $request, AssainissementService $assainissementService)
    {
        $infoPhotosInfra = [];
        $response = new Response();
        if (isset($id) && !empty($id)) {
            $infoPhotosInfra = $assainissementService->getPhotoInfraInfo($id);
    ...
    /**
     * @Route("/api/assainissement/deletephoto", name="assainissement_delete_photo", methods={"POST"})
     */
    public function deletePhoto(Request $request, AssainissementService $assainissementService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $infoPhotosInfra = $assainissementService->getPhotoInfraInfo($idInfra);
                ...
                $directory = $this->pathImageAssainissement . $indexPhoto."/";
                ...
                if (array_key_exists($indexPhoto, $oldPhotosInfra)) {
                    ...
                    if (isset($setUpdate) && !empty($setUpdate)) {
                        $idInfra = $assainissementService->addInfrastructurePhoto($idInfra, $setUpdate);
                    }
                   
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Photo assainissement route deleted_successfull"
                    ]));
                } else {
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Pas de photo assainissement route supprimer"
                    ]));
                }
    ...
    /**
     * @Route("/api/assainissement/updatephoto", name="assainissement_update_photo", methods={"POST"})
     */
    public function updatePhoto(Request $request, AssainissementService $assainissementService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if(!is_dir($this->pathImageAssainissement)) {
                mkdir($this->pathImageAssainissement, 0777, true);
            }
          
            $directory1 = $this->pathImageAssainissement . "photo1/";
            ...
            if (null != $uploadedFile1 && "null" != $uploadedFile1 && "undefined" != $uploadedFile1) {
                ...
                $data['photo1'] = $this->pathForNamePhotoAssainissement."photo1/" .$nomPhoto1;
            ...
            $directory2 = $this->pathImageAssainissement . "photo2/";
            ...
            if (null != $uploadedFile2 && "null" != $uploadedFile2 && "undefined" != $uploadedFile2) {
                ...
                $data['photo2'] = $this->pathForNamePhotoAssainissement."photo2/" .$nomPhoto2;
            ...
            $directory3 = $this->pathImageAssainissement . "photo3/";
            ...
            if (null != $uploadedFile3 && "null" != $uploadedFile3 && "undefined" != $uploadedFile3) {
                ...
                $data['photo3'] = $this->pathForNamePhotoAssainissement."photo3/" .$nomPhoto3;
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "Photo assainissement route updated_successfull"
            ]));
    ...
    /**
     * @Route("/api/assainissement/add", name="assainissement_add", methods={"POST"})
     */
    public function create(Request $request, AssainissementService $assainissementService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            $data['etatFosse'] = $request->get('etatFosse');
            $data['etatBassin'] = $request->get('etatBassin');
            $data['precisionTypeWc'] = $request->get('precisionTypeWc');
            ...
            $idInfra = $assainissementService->addInfrastructure($data);

            if ($idInfra != false) {
                // add situation et etat
                //$idEtat = $assainissementService->addInfrastructureRouteEtat($idInfra, $data);

                $idEtat = $assainissementService->addInfrastructureSituation($idInfra, $data);

                $idDataCollected = $assainissementService->addInfrastructureDonneCollecte($idInfra, $data);

                /*$idStructure = $assainissementService->addInfrastructureRouteStructure($idInfra, $data);

                $idAccotement = $assainissementService->addInfrastructureRouteAccotement($idInfra, $data);

                $idFosse = $assainissementService->addInfrastructureRouteFosse($idInfra, $data);*/
                ...
                $idFoncier = $assainissementService->addInfrastructureRouteFoncier($idInfra, $data);*/
                ...
                if (null != $request->get('hasTravaux') && ($request->get('hasTravaux') == true || $request->get('hasTravaux') == "true") && "false" != $request->get('hasTravaux')) {
                    ...
                    $idTravaux = $assainissementService->addInfrastructureTravaux($idInfra, $data);
                }
                ...
                $idFourniture = $assainissementService->addInfrastructureRouteFourniture($idInfra, $data);*/
                // Etudes
                ...
                // $data['precisionConsistanceContratEtude'] = $request->get('precisionConsistanceContratEtude');
                    ...
                    $idEtude = $assainissementService->addInfrastructureEtudes($idInfra, $data);
                }
                
                /**
                 * End Administrative data
                */
                //$idDonneAnnexe = $assainissementService->addInfrastructureEducationDonneAnnexe($idInfra, $data);
            }

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "assainissement route created_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'situation');
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'data');
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'surface');
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'structure');
            
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'accotement');
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'fosse');
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'foncier');
           
            $assainissementService->cleanTablesByIdInfrastructure($idInfra, 'fourniture');*/
    ...
    /**
     * @Route("/api/infra/assainissement/liste", name="assainissement_list", methods={"GET"})
     */
    public function listeassainissement(Request $request, AssainissementService $assainissementService)
    {    
        $response = new Response();
        
        try {

            $routes = $assainissementService->getAllInfrastructures();

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "assainissement route list_successfull",
                'pathImage' => $this->pathImage,
                'data' => $routes
            ]));
    ...
    /**
     * @Route("/api/infra/assainissement/liste/minifie", name="assainissement_list_minifie", methods={"GET"})
     */
    public function listeassainissementMinifie(Request $request, AssainissementService $assainissementService)
    {    
        $response = new Response();
        
        try {

            $routes = $assainissementService->getAllInfrastructuresMinifie();
    ...
    /**
     * @Route("/api/infra/assainissement/info", name="assainissement_info", methods={"POST"})
     */
    public function getOneInfraInfo(Request $request, AssainissementService $assainissementService)
    {    
        $response = new Response();
        ...
        try {
            ...
            $routes = $assainissementService->getOneInfraInfo(intval($infraId));

            $routesInfrastructure = $assainissementService->getAllyRouteInfoMinifie();
            $infoRoutes = [];
    ...
    /**
     * @Route("/api/assainissement/update", name="assainissement_update", methods={"POST"})
     */
    public function update(Request $request, AssainissementService $assainissementService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                if (array_key_exists('infrastructure', $data) && count($data['infrastructure']) > 0) {
                    ...
                    $allCategories = $assainissementService->getAllCategorieInfra();
                    ...
                    $idInfra = $assainissementService->updateInfrastructure($idInfra, $updateColonneInfra);
                    }
                }
                // Situation
                ...
                if (array_key_exists('situations', $data) && count($data['situations']) > 0) {
                    ...
                    if ($idSituation == 0) {
                        $idSituation = $assainissementService->addInfoInTableByInfrastructure('t_as_03_situation', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtat) && !empty($updateColonneEtat)) {
                        $idSituation = $assainissementService->updateInfrastructureTables('t_as_03_situation', $idSituation, $updateColonneEtat);
                        }
                    } 
                ...
                if (array_key_exists('data_collecte', $data) && count($data['data_collecte']) > 0) {
                    ...
                    if ($idData == 0) {
                        $idData = $assainissementService->addInfoInTableByInfrastructure('t_as_06_donnees_collectees', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneData) && !empty($updateColonneData)) {
                        $idData = $assainissementService->updateInfrastructureTables('t_as_06_donnees_collectees', $idData, $updateColonneData);
                        }
                    }
                }
                // Travaux
                ...
                if (array_key_exists('travaux', $data) && count($data['travaux']) > 0) {
                    ...
                    if ($idTravaux == 0) {
                        $idTravaux = $assainissementService->addInfoInTableByInfrastructure('t_bc_05_travaux', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneTravaux) && !empty($updateColonneTravaux)) {
                        $idTravaux = $assainissementService->updateInfrastructureTables('t_bc_05_travaux', $idTravaux, $updateColonneTravaux);
                        }
                    }
                }
                ...
                if (array_key_exists('etudes', $data) && count($data['etudes']) > 0) {
                    ...
                    if ($idEtudes == 0) {
                        $idEtudes = $assainissementService->addInfoInTableByInfrastructure('t_bc_07_etudes', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtudes) && !empty($updateColonneEtudes)) {
                        $idEtudes = $assainissementService->updateInfrastructureTables('t_bc_07_etudes', $idEtudes, $updateColonneEtudes);
                        }
                    }
                }
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "assainissement update_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            //$assainissementService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            //$assainissementService->cleanTablesByIdInfrastructure($idInfra, 'etat');
            //$assainissementService->cleanTablesByIdInfrastructure($idInfra, 'data');
            //$assainissementService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            //$assainissementService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
