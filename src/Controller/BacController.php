...
class BacController extends AbstractController
{
    ...
    private $pathImageBac = null;
    private $pathPublic = null;
    private $pathForNamePhotobac = null;
    private $kernelInterface;
    private $directoryCopy = null;
    private const nameRepertoireImage = 'bc_bac/t_bc_01_infrastructure/';
    ...
    public function __construct(ParameterBagInterface $params, KernelInterface  $kernelInterface) {
        ...
        $this->pathImageBac = $params->get('pathImageBac');
        $this->pathPublic = $params->get('pathPublic');
        $this->pathForNamePhotobac = $params->get('pathForNamePhotoBac');
    ...
    {    
        ...
        try {
            ...
            // Data collecte
            $data['etatMoteurBac'] = $request->get('etatMoteurBac');
            ...
            $idInfra = $bacService->addInfrastructure($data);

            if ($idInfra != false) {
                // add situation et etat
                //$idEtat = $bacService->addInfrastructureRouteEtat($idInfra, $data);

                $idEtat = $bacService->addInfrastructureSituation($idInfra, $data);

                $idDataCollected = $bacService->addInfrastructureDonneCollecte($idInfra, $data);

                /*$idStructure = $bacService->addInfrastructureRouteStructure($idInfra, $data);

                $idAccotement = $bacService->addInfrastructureRouteAccotement($idInfra, $data);

                $idFosse = $bacService->addInfrastructureRouteFosse($idInfra, $data);*/
                ...
                $idFoncier = $bacService->addInfrastructureRouteFoncier($idInfra, $data);*/
                ...
                if (null != $request->get('hasTravaux') && ($request->get('hasTravaux') == true || $request->get('hasTravaux') == "true") && "false" != $request->get('hasTravaux')) {
                    ...
                    $idTravaux = $bacService->addInfrastructureTravaux($idInfra, $data);
                }
                ...
                $idFourniture = $bacService->addInfrastructureRouteFourniture($idInfra, $data);*/
                // Etudes
                ...
                // $data['precisionConsistanceContratEtude'] = $request->get('precisionConsistanceContratEtude');
                    ...
                    $idEtude = $bacService->addInfrastructureEtudes($idInfra, $data);
                }
                
                /**
                 * End Administrative data
                */
                //$idDonneAnnexe = $bacService->addInfrastructureEducationDonneAnnexe($idInfra, $data);
            }

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "bac route created_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'situation');
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'data');
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'surface');
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'structure');
            
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'accotement');
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'fosse');
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'foncier');
           
            $bacService->cleanTablesByIdInfrastructure($idInfra, 'fourniture');*/
    ...
    {    
        ...
        try {
            ...
            $routes = $bacService->getAllInfrastructures();

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "bac route list_successfull",
    ...
    {    
        ...
        try {
            ...
            $routes = $bacService->getAllInfrastructuresMinifie();
    ...
    {    
        ...
        try {
            ...
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                if (array_key_exists('infrastructure', $data) && count($data['infrastructure']) > 0) {
                    ...
                    $idInfra = $bacService->updateInfrastructure($idInfra, $updateColonneInfra);
                    }
                }
                // Situation
                ...
                if (array_key_exists('situations', $data) && count($data['situations']) > 0) {
                    ...
                    if ($idSituation == 0) {
                        ...
                        $idSituation = $bacService->updateInfrastructureTables('t_bc_02_situation', $idSituation, $updateColonneEtat);
                        }
                    } 
                ...
                if (array_key_exists('data_collecte', $data) && count($data['data_collecte']) > 0) {
                    ...
                    if ($idData == 0) {
                        ...
                        $idData = $bacService->updateInfrastructureTables('t_bc_04_donnees_collectees', $idData, $updateColonneData);
                        }
                    }
                }
                // Travaux
                ...
                if (array_key_exists('travaux', $data) && count($data['travaux']) > 0) {
                    ...
                    if ($idTravaux == 0) {
                        ...
                        $idTravaux = $bacService->updateInfrastructureTables('t_bc_05_travaux', $idTravaux, $updateColonneTravaux);
                        }
                    }
                }
                ...
                if (array_key_exists('etudes', $data) && count($data['etudes']) > 0) {
                    ...
                    if ($idEtudes == 0) {
                        ...
                        $idEtudes = $bacService->updateInfrastructureTables('t_bc_07_etudes', $idEtudes, $updateColonneEtudes);
                        }
                    }
                }
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "bac update_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            //$bacService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            //$bacService->cleanTablesByIdInfrastructure($idInfra, 'etat');
            //$bacService->cleanTablesByIdInfrastructure($idInfra, 'data');
            //$bacService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            //$bacService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
