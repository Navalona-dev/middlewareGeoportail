    ...
    { 
        ...
        try {
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $directory = $this->pathImageCunette . $indexPhoto."/";
                ...
                if (array_key_exists($indexPhoto, $oldPhotosInfra)) {
                    ...
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Photo cunette route deleted_successfull"
                    ]));
                } else {
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Pas de photo cunette route supprimer"
                    ]));
                }
                        ...
                        if (in_array($colonne, $colonneInteger)) {
                            ...
                            if ($colonne == "nom_de_la_route_a_qui_il_est_rattache") {
                                if ($value != "null" && $value != "undefined" && $value != "") {
                                    $infoYlisteRoute = $CunetteService->getInfoyRouteInfoMinifie($value);
                                    if ($infoYlisteRoute != false && count($infoYlisteRoute) > 0) {
