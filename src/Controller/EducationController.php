...
class EducationController extends AbstractController
{
    ...
    private const nameRepertoireImage = 'ec_ecole/t_ec_01_infrastructure/';
    ...
    /**
     * @Route("/api/education/getphoto/{id}", name="infra_education_photo", methods={"GET"})
     */
    public function getPhotosByInfra($id, Request $request, EducationService $educationService)
    {
        $infoPhotosInfra = [];
        $response = new Response();
        if (isset($id) && !empty($id)) {
            $infoPhotosInfra = $educationService->getPhotoInfraInfo($id);
    ...
    /**
     * @Route("/api/education/deletephoto", name="education_delete_photo", methods={"POST"})
     */
    public function deletePhoto(Request $request, EducationService $educationService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $directory = $this->pathImageEducation . $indexPhoto."/";
                ...
                if (array_key_exists($indexPhoto, $oldPhotosInfra)) {
                    ...
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Photo education route deleted_successfull"
                    ]));
                } else {
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Pas de photo education route supprimer"
                    ]));
                }
    ...
    { 
        ...
        try {
            ...
            if(!is_dir($this->pathImageEducation)) {
                mkdir($this->pathImageEducation, 0777, true);
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "Photo education route updated_successfull"
            ]));
    ...
    {   
        ...
        try {
            ...
            if ($request->get('categorie') != "null" && $request->get('categorie') != "undefined") {
                $allCategories = $educationService->getAllCategorieInfra();
            ...
            $data['existenceTerrainFoot'] = $request->get('existenceTerrainFoot');
            $data['etatTerrainFoot'] = $request->get('etatTerrainFoot');
            $data['existenceTerrainMixte'] = $request->get('existenceTerrainMixte');
            ...
            $data['existenceCantine' ] = $request->get('existenceCantine');
            $data['nombreEnseignant' ] = $request->get('nombreEnseignant');
            $data['nombreEleve' ] = $request->get('nombreEleve');
            ...
            $idInfra = $educationService->addInfrastructure($data);

            if ($idInfra != false) {
                // add situation et etat
                //$idEtat = $educationService->addInfrastructureRouteEtat($idInfra, $data);

                $idEtat = $educationService->addInfrastructureSituation($idInfra, $data);

                $idDataCollected = $educationService->addInfrastructureDonneCollecte($idInfra, $data);

                /*$idStructure = $educationService->addInfrastructureRouteStructure($idInfra, $data);

                $idAccotement = $educationService->addInfrastructureRouteAccotement($idInfra, $data);

                $idFosse = $educationService->addInfrastructureRouteFosse($idInfra, $data);*/
                ...
                $idFoncier = $educationService->addInfrastructureRouteFoncier($idInfra, $data);*/
                ...
                if (null != $request->get('hasTravaux') && ($request->get('hasTravaux') == true || $request->get('hasTravaux') == "true") && "false" != $request->get('hasTravaux')) {
                    ...
                    $idTravaux = $educationService->addInfrastructureTravaux($idInfra, $data);
                }
                ...
                $idFourniture = $educationService->addInfrastructureRouteFourniture($idInfra, $data);*/
                // Etudes
                ...
                // $data['precisionConsistanceContratEtude'] = $request->get('precisionConsistanceContratEtude');
                    ...
                    $idEtude = $educationService->addInfrastructureEtudes($idInfra, $data);
                }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "education route created_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'situation');
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'data');
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'surface');
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'structure');
            
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'accotement');
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'fosse');
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'foncier');
           
            $educationService->cleanTablesByIdInfrastructure($idInfra, 'fourniture');*/
     ...
     */
    public function listeeducation(Request $request, EducationService $educationService)
    {    
        $response = new Response();
        
        try {

            $routes = $educationService->getAllInfrastructures();

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "education route list_successfull",
                'pathImage' => $this->pathImage,
                'data' => $routes
            ]));
    ...
    /**
     * @Route("/api/infra/education/liste/minifie", name="education_list_minifie", methods={"GET"})
     */
    public function listeeducationMinifie(Request $request, EducationService $educationService)
    {    
        $response = new Response();
        
        try {

            $routes = $educationService->getAllInfrastructuresMinifie();
    ...
    /**
     * @Route("/api/infra/education/info", name="education_info", methods={"POST"})
     */
    public function getOneInfraInfo(Request $request, EducationService $educationService)
    {    
        $response = new Response();
        ...
        try {
            ...
            $routes = $educationService->getOneInfraInfo(intval($infraId));
    ...
    /**
     * @Route("/api/education/update", name="education_update", methods={"POST"})
     */
    public function update(Request $request, EducationService $educationService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $colonneInteger = ['nombre_enseignant', 'nombre_eleve', 'id', 'gid', 'id_infrastructure', 'id_controle_surveillance', 'montant', 'id_titulaire', 'id_ingenieurs_reception_provisoire',
                ...
                if (array_key_exists('infrastructure', $data) && count($data['infrastructure']) > 0) {
                    ...
                    $idInfra = $educationService->updateInfrastructure($idInfra, $updateColonneInfra);
                    }
                }
                // Situation
                ...
                if (array_key_exists('situations', $data) && count($data['situations']) > 0) {
                    ...
                    if ($idSituation == 0) {
                        $idSituation = $educationService->addInfoInTableByInfrastructure('t_ec_03_situation', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtat) && !empty($updateColonneEtat)) {
                        $idSituation = $educationService->updateInfrastructureTables('t_ec_03_situation', $idSituation, $updateColonneEtat);
                        }
                    } 
                ...
                if (array_key_exists('data_collecte', $data) && count($data['data_collecte']) > 0) {
                    ...
                    if ($idData == 0) {
                        $idData = $educationService->addInfoInTableByInfrastructure('t_ec_06_donnees_collectees', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneData) && !empty($updateColonneData)) {
                        $idData = $educationService->updateInfrastructureTables('t_ec_06_donnees_collectees', $idData, $updateColonneData);
                        }
                    }
                }
                // Travaux
                ...
                if (array_key_exists('travaux', $data) && count($data['travaux']) > 0) {
                    ...
                    if ($idTravaux == 0) {
                        $idTravaux = $educationService->addInfoInTableByInfrastructure('t_ec_08_travaux', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneTravaux) && !empty($updateColonneTravaux)) {
                        $idTravaux = $educationService->updateInfrastructureTables('t_ec_08_travaux', $idTravaux, $updateColonneTravaux);
                        }
                    }
                }
                ...
                if (array_key_exists('etudes', $data) && count($data['etudes']) > 0) {
                    ...
                    if ($idEtudes == 0) {
                        $idEtudes = $educationService->addInfoInTableByInfrastructure('t_ec_12_etudes', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtudes) && !empty($updateColonneEtudes)) {
                        $idEtudes = $educationService->updateInfrastructureTables('t_ec_12_etudes', $idEtudes, $updateColonneEtudes);
                        }
                    }
                }
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "education update_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            //$educationService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            //$educationService->cleanTablesByIdInfrastructure($idInfra, 'etat');
            //$educationService->cleanTablesByIdInfrastructure($idInfra, 'data');
            //$educationService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            //$educationService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
