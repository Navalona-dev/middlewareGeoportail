...
use App\Service\GareferoviaireService;
...
class GareferoviaireController extends AbstractController
{
    private $pathImage = null;
    private $pathImagegareferoviaire = null;
    private $pathPublic = null;
    private $pathForNamePhotogareferoviaire = null;
    private $kernelInterface;
    private $directoryCopy = null;
    private const nameRepertoireImage = 'gf_gare_ferroviaire/t_gf_01_infrastructure/';
    ...
    public function __construct(ParameterBagInterface $params, KernelInterface  $kernelInterface) {
        ...
        $this->pathImageGareferoviaire = $params->get('pathImageGareferoviaire');
        $this->pathPublic = $params->get('pathPublic');
        $this->pathForNamePhotoGareferoviaire = $params->get('pathForNamePhotoGareferoviaire');
    ...
    /**
     * @Route("/api/gareferoviaire/getphoto/{id}", name="infra_gareferoviaire_photo", methods={"GET"})
     */
    public function getPhotosByInfra($id, Request $request, GareferoviaireService $gareferoviaireService)
    {
        $infoPhotosInfra = [];
        $response = new Response();
        if (isset($id) && !empty($id)) {
            $infoPhotosInfra = $gareferoviaireService->getPhotoInfraInfo($id);
    ...
    /**
     * @Route("/api/gareferoviaire/deletephoto", name="gareferoviaire_delete_photo", methods={"POST"})
     */
    public function deletePhoto(Request $request, GareferoviaireService $gareferoviaireService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $infoPhotosInfra = $gareferoviaireService->getPhotoInfraInfo($idInfra);
                ...
                $directory = $this->pathImageGareferoviaire . $indexPhoto."/";
                ...
                if (array_key_exists($indexPhoto, $oldPhotosInfra)) {
                    ...
                    if (isset($setUpdate) && !empty($setUpdate)) {
                        $idInfra = $gareferoviaireService->addInfrastructurePhoto($idInfra, $setUpdate);
                    }
                   
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Photo gareferoviaire route deleted_successfull"
                    ]));
                } else {
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Pas de photo gareferoviaire route supprimer"
                    ]));
                }
    ...
    /**
     * @Route("/api/gareferoviaire/updatephoto", name="gareferoviaire_update_photo", methods={"POST"})
     */
    public function updatePhoto(Request $request, GareferoviaireService $gareferoviaireService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if(!is_dir($this->pathImageGareferoviaire)) {
                mkdir($this->pathImageGareferoviaire, 0777, true);
            }
          
            $directory1 = $this->pathImageGareferoviaire . "photo1/";
            ...
            if (null != $uploadedFile1 && "null" != $uploadedFile1 && "undefined" != $uploadedFile1) {
                ...
                $data['photo1'] = $this->pathForNamePhotoGareferoviaire."photo1/" .$nomPhoto1;
            ...
            $directory2 = $this->pathImageGareferoviaire . "photo2/";
            ...
            if (null != $uploadedFile2 && "null" != $uploadedFile2 && "undefined" != $uploadedFile2) {
                ...
                $data['photo2'] = $this->pathForNamePhotoGareferoviaire."photo2/" .$nomPhoto2;
            ...
            $directory3 = $this->pathImageGareferoviaire . "photo3/";
            ...
            if (null != $uploadedFile3 && "null" != $uploadedFile3 && "undefined" != $uploadedFile3) {
                ...
                $data['photo3'] = $this->pathForNamePhotoGareferoviaire."photo3/" .$nomPhoto3;
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "Photo gareferoviaire route updated_successfull"
            ]));
    ...
    /**
     * @Route("/api/gareferoviaire/add", name="gareferoviaire_add", methods={"POST"})
     */
    public function create(Request $request, GareferoviaireService $gareferoviaireService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            // Data collecte
            $data['existenceSystemeAiguillage'] = $request->get('existenceSystemeAiguillage');
            $data['emplacementSystèmeAiguillage1'] = $request->get('emplacementSystèmeAiguillage1');
            $data['etatSystèmeAiguillage1'] = $request->get('etatSystèmeAiguillage1');
            $data['emplacementSystèmeAiguillage2'] = $request->get('emplacementSystèmeAiguillage2');
            $data['etatSystèmeAiguillage2'] = $request->get('etatSystèmeAiguillage2');
            ...
            $idInfra = $gareferoviaireService->addInfrastructure($data);

            if ($idInfra != false) {
                // add situation et etat
                //$idEtat = $gareferoviaireService->addInfrastructureRouteEtat($idInfra, $data);

                $idEtat = $gareferoviaireService->addInfrastructureSituation($idInfra, $data);

                $idDataCollected = $gareferoviaireService->addInfrastructureDonneCollecte($idInfra, $data);

                /*$idStructure = $gareferoviaireService->addInfrastructureRouteStructure($idInfra, $data);

                $idAccotement = $gareferoviaireService->addInfrastructureRouteAccotement($idInfra, $data);

                $idFosse = $gareferoviaireService->addInfrastructureRouteFosse($idInfra, $data);*/
                ...
                $idFoncier = $gareferoviaireService->addInfrastructureRouteFoncier($idInfra, $data);*/
                ...
                if (null != $request->get('hasTravaux') && ($request->get('hasTravaux') == true || $request->get('hasTravaux') == "true") && "false" != $request->get('hasTravaux')) {
                    ...
                    $idTravaux = $gareferoviaireService->addInfrastructureTravaux($idInfra, $data);
                }
                ...
                $idFourniture = $gareferoviaireService->addInfrastructureRouteFourniture($idInfra, $data);*/
                // Etudes
                ...
                // $data['precisionConsistanceContratEtude'] = $request->get('precisionConsistanceContratEtude');
                    ...
                    $idEtude = $gareferoviaireService->addInfrastructureEtudes($idInfra, $data);
                }
                
                /**
                 * End Administrative data
                */
                //$idDonneAnnexe = $gareferoviaireService->addInfrastructureEducationDonneAnnexe($idInfra, $data);
            }

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "gareferoviaire route created_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'situation');
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'data');
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'surface');
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'structure');
            
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'accotement');
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'fosse');
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'foncier');
           
            $gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'fourniture');*/
    ...
    /**
     * @Route("/api/infra/gareferoviaire/liste", name="gareferoviaire_list", methods={"GET"})
     */
    public function listegareferoviaire(Request $request, GareferoviaireService $gareferoviaireService)
    {    
        $response = new Response();
        
        try {

            $routes = $gareferoviaireService->getAllInfrastructures();

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "gareferoviaire route list_successfull",
                'pathImage' => $this->pathImage,
                'data' => $routes
            ]));
    ...
    /**
     * @Route("/api/infra/gareferoviaire/liste/minifie", name="gareferoviaire_list_minifie", methods={"GET"})
     */
    public function listegareferoviaireMinifie(Request $request, GareferoviaireService $gareferoviaireService)
    {    
        $response = new Response();
        
        try {

            $routes = $gareferoviaireService->getAllInfrastructuresMinifie();
    ...
    /**
     * @Route("/api/infra/gareferoviaire/info", name="gareferoviaire_info", methods={"POST"})
     */
    public function getOneInfraInfo(Request $request, GareferoviaireService $gareferoviaireService)
    {    
        $response = new Response();
        ...
        try {
            ...
            $routes = $gareferoviaireService->getOneInfraInfo(intval($infraId));
    ...
    /**
     * @Route("/api/gareferoviaire/update", name="gareferoviaire_update", methods={"POST"})
     */
    public function update(Request $request, GareferoviaireService $gareferoviaireService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                if (array_key_exists('infrastructure', $data) && count($data['infrastructure']) > 0) {
                    ...
                    $allCategories = $gareferoviaireService->getAllCategorieInfra();
                    ...
                    $idInfra = $gareferoviaireService->updateInfrastructure($idInfra, $updateColonneInfra);
                    }
                }
                // Situation
                ...
                if (array_key_exists('situations', $data) && count($data['situations']) > 0) {
                    ...
                    if ($idSituation == 0) {
                        $idSituation = $gareferoviaireService->addInfoInTableByInfrastructure('t_gf_03_situation', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtat) && !empty($updateColonneEtat)) {
                        $idSituation = $gareferoviaireService->updateInfrastructureTables('t_gf_03_situation', $idSituation, $updateColonneEtat);
                        }
                    } 
                ...
                if (array_key_exists('data_collecte', $data) && count($data['data_collecte']) > 0) {
                    ...
                    if ($idData == 0) {
                        $idData = $gareferoviaireService->addInfoInTableByInfrastructure('t_gf_06_donnees_collectees', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneData) && !empty($updateColonneData)) {
                        $idData = $gareferoviaireService->updateInfrastructureTables('t_gf_06_donnees_collectees', $idData, $updateColonneData);
                        }
                    }
                }
                // Travaux
                ...
                if (array_key_exists('travaux', $data) && count($data['travaux']) > 0) {
                    ...
                    if ($idTravaux == 0) {
                        $idTravaux = $gareferoviaireService->addInfoInTableByInfrastructure('t_bc_05_travaux', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneTravaux) && !empty($updateColonneTravaux)) {
                        $idTravaux = $gareferoviaireService->updateInfrastructureTables('t_bc_05_travaux', $idTravaux, $updateColonneTravaux);
                        }
                    }
                }
                ...
                if (array_key_exists('etudes', $data) && count($data['etudes']) > 0) {
                    ...
                    if ($idEtudes == 0) {
                        $idEtudes = $gareferoviaireService->addInfoInTableByInfrastructure('t_bc_07_etudes', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtudes) && !empty($updateColonneEtudes)) {
                        $idEtudes = $gareferoviaireService->updateInfrastructureTables('t_bc_07_etudes', $idEtudes, $updateColonneEtudes);
                        }
                    }
                }
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "gareferoviaire update_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            //$gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            //$gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'etat');
            //$gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'data');
            //$gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            //$gareferoviaireService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
