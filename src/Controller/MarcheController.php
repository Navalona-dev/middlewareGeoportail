...
class MarcheController extends AbstractController
{
    ...
    private $pathImageInfrastructuremarche = null;
    private $pathPublic = null;
    private $pathForNameInfrastructuremarche = null;
    private $kernelInterface;
    private $directoryCopy = null;
    private const nameRepertoireImage = 'ma_marche/t_ma_01_infrastructure/';
    ...
    public function __construct(ParameterBagInterface $params, KernelInterface  $kernelInterface) {
        ...
        $this->pathImageInfrastructuremarche = $params->get('pathImageInfrastructuremarche');
        $this->pathPublic = $params->get('pathPublic');
        $this->pathForNameInfrastructuremarche = $params->get('pathForNameInfrastructuremarche');
    ...
    /**
     * @Route("/api/marche/getphoto/{id}", name="infra_marche_photo", methods={"GET"})
     */
    public function getPhotosByInfra($id, Request $request, MarcheService $marcheService)
    {
        $infoPhotosInfra = [];
        $response = new Response();
        if (isset($id) && !empty($id)) {
            $infoPhotosInfra = $marcheService->getPhotoInfraInfo($id);
    ...
    /**
     * @Route("/api/marche/deletephoto", name="marche_delete_photo", methods={"POST"})
     */
    public function deletePhoto(Request $request, MarcheService $marcheService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $infoPhotosInfra = $marcheService->getPhotoInfraInfo($idInfra);
                ...
                $directory = $this->pathImageInfrastructuremarche . $indexPhoto."/";
                ...
                if (array_key_exists($indexPhoto, $oldPhotosInfra)) {
                    ...
                    if (isset($setUpdate) && !empty($setUpdate)) {
                        $idInfra = $marcheService->addInfrastructurePhoto($idInfra, $setUpdate);
                    }
                   
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Photo marche route deleted_successfull"
                    ]));
                } else {
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Pas de photo marche route supprimer"
                    ]));
                }
    ...
    /**
     * @Route("/api/marche/updatephoto", name="marche_update_photo", methods={"POST"})
     */
    public function updatePhoto(Request $request, MarcheService $marcheService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if(!is_dir($this->pathImageInfrastructuremarche)) {
                mkdir($this->pathImageInfrastructuremarche, 0777, true);
            }
          
            $directory1 = $this->pathImageInfrastructuremarche . "photo1/";
            ...
            if (null != $uploadedFile1 && "null" != $uploadedFile1 && "undefined" != $uploadedFile1) {
                ...
                $data['photo1'] = $this->pathForNameInfrastructuremarche."photo1/" .$nomPhoto1;
            ...
            $directory2 = $this->pathImageInfrastructuremarche . "photo2/";
            ...
            if (null != $uploadedFile2 && "null" != $uploadedFile2 && "undefined" != $uploadedFile2) {
                ...
                $data['photo2'] = $this->pathForNameInfrastructuremarche."photo2/" .$nomPhoto2;
            ...
            $directory3 = $this->pathImageInfrastructuremarche . "photo3/";
            ...
            if (null != $uploadedFile3 && "null" != $uploadedFile3 && "undefined" != $uploadedFile3) {
                ...
                $data['photo3'] = $this->pathForNameInfrastructuremarche."photo3/" .$nomPhoto3;
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "Photo marche route updated_successfull"
            ]));
    ...
    /**
     * @Route("/api/marche/add", name="marche_add", methods={"POST"})
     */
    public function create(Request $request, MarcheService $marcheService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            $data['marchandiseVendue' ] = $request->get('marchandiseVendue');
            $data['lundiJourMarche' ] = $request->get('lundiJourMarche');
            $data['mardiJourMarche' ] = $request->get('mardiJourMarche');
            $data['mercrediJourMarche' ] = $request->get('mercrediJourMarche');
            $data['jeudiJourMarche' ] = $request->get('jeudiJourMarche');
            $data['vendrediJourMarche' ] = $request->get('vendrediJourMarche');
            $data['samediJourMarche' ] = $request->get('samediJourMarche');
            $data['dimancheJourMarche' ] = $request->get('dimancheJourMarche');
            ...
            if ($request->get('categorie') != "null" && $request->get('categorie') != "undefined") {
                $allCategories = $marcheService->getAllCategorieInfra();
            ...
            // Data collecte
            $data['existenceEtales'] = $request->get('existenceEtales');
            $data['etatEtales'] = $request->get('etatEtales');
            ...
            $idInfra = $marcheService->addInfrastructure($data);

            if ($idInfra != false) {
                // add situation et etat
                //$idEtat = $marcheService->addInfrastructureRouteEtat($idInfra, $data);

                $idEtat = $marcheService->addInfrastructureSituation($idInfra, $data);

                $idDataCollected = $marcheService->addInfrastructureDonneCollecte($idInfra, $data);

                /*$idStructure = $marcheService->addInfrastructureRouteStructure($idInfra, $data);

                $idAccotement = $marcheService->addInfrastructureRouteAccotement($idInfra, $data);

                $idFosse = $marcheService->addInfrastructureRouteFosse($idInfra, $data);*/
                ...
                $idFoncier = $marcheService->addInfrastructureRouteFoncier($idInfra, $data);*/
                ...
                if (null != $request->get('hasTravaux') && ($request->get('hasTravaux') == true || $request->get('hasTravaux') == "true") && "false" != $request->get('hasTravaux')) {
                    ...
                    $idTravaux = $marcheService->addInfrastructureTravaux($idInfra, $data);
                }
                ...
                $idFourniture = $marcheService->addInfrastructureRouteFourniture($idInfra, $data);*/
                // Etudes
                ...
                // $data['precisionConsistanceContratEtude'] = $request->get('precisionConsistanceContratEtude');
                    ...
                    $idEtude = $marcheService->addInfrastructureEtudes($idInfra, $data);
                }
                
                /**
                 * End Administrative data
                */
                //$idDonneAnnexe = $marcheService->addInfrastructureEducationDonneAnnexe($idInfra, $data);
            }

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "marche route created_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'situation');
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'data');
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'surface');
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'structure');
            
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'accotement');
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'fosse');
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'foncier');
           
            $marcheService->cleanTablesByIdInfrastructure($idInfra, 'fourniture');*/
    ...
    /**
     * @Route("/api/infra/marche/liste", name="marche_list", methods={"GET"})
     */
    public function listemarche(Request $request, MarcheService $marcheService)
    {    
        $response = new Response();
        
        try {

            $routes = $marcheService->getAllInfrastructures();

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "marche route list_successfull",
                'pathImage' => $this->pathImage,
                'data' => $routes
            ]));
    ...
    /**
     * @Route("/api/infra/marche/liste/minifie", name="marche_list_minifie", methods={"GET"})
     */
    public function listemarcheMinifie(Request $request, MarcheService $marcheService)
    {    
        $response = new Response();
        
        try {

            $routes = $marcheService->getAllInfrastructuresMinifie();
    ...
    /**
     * @Route("/api/infra/marche/info", name="marche_info", methods={"POST"})
     */
    public function getOneInfraInfo(Request $request, MarcheService $marcheService)
    {    
        $response = new Response();
        ...
        try {
            ...
            $routes = $marcheService->getOneInfraInfo(intval($infraId));
    ...
    /**
     * @Route("/api/marche/update", name="marche_update", methods={"POST"})
     */
    public function update(Request $request, MarcheService $marcheService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                if (array_key_exists('infrastructure', $data) && count($data['infrastructure']) > 0) {
                    ...
                    $idInfra = $marcheService->updateInfrastructure($idInfra, $updateColonneInfra);
                    }
                }
                // Situation
                ...
                if (array_key_exists('situations', $data) && count($data['situations']) > 0) {
                    ...
                    if ($idSituation == 0) {
                        $idSituation = $marcheService->addInfoInTableByInfrastructure('t_ma_03_situation', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtat) && !empty($updateColonneEtat)) {
                        $idSituation = $marcheService->updateInfrastructureTables('t_ma_03_situation', $idSituation, $updateColonneEtat);
                        }
                    } 
                ...
                if (array_key_exists('data_collecte', $data) && count($data['data_collecte']) > 0) {
                    ...
                    if ($idData == 0) {
                        $idData = $marcheService->addInfoInTableByInfrastructure('t_ma_06_donnees_collectees', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneData) && !empty($updateColonneData)) {
                        $idData = $marcheService->updateInfrastructureTables('t_ma_06_donnees_collectees', $idData, $updateColonneData);
                        }
                    }
                }
                // Travaux
                ...
                if (array_key_exists('travaux', $data) && count($data['travaux']) > 0) {
                    ...
                    if ($idTravaux == 0) {
                        $idTravaux = $marcheService->addInfoInTableByInfrastructure('t_ma_08_travaux', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneTravaux) && !empty($updateColonneTravaux)) {
                        $idTravaux = $marcheService->updateInfrastructureTables('t_ma_08_travaux', $idTravaux, $updateColonneTravaux);
                        }
                    }
                }
                ...
                if (array_key_exists('etudes', $data) && count($data['etudes']) > 0) {
                    ...
                    if ($idEtudes == 0) {
                        $idEtudes = $marcheService->addInfoInTableByInfrastructure('t_ma_10_etudes', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtudes) && !empty($updateColonneEtudes)) {
                        $idEtudes = $marcheService->updateInfrastructureTables('t_ma_10_etudes', $idEtudes, $updateColonneEtudes);
                        }
                    }
                }
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "marche update_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            //$marcheService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            //$marcheService->cleanTablesByIdInfrastructure($idInfra, 'etat');
            //$marcheService->cleanTablesByIdInfrastructure($idInfra, 'data');
            //$marcheService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            //$marcheService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
