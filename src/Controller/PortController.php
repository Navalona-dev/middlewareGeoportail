...
class PortController extends AbstractController
{
    ...
    private $pathImagePort = null;
    private $pathPublic = null;
    private $pathForNamePort = null;
    private $kernelInterface;
    private $directoryCopy = null;
    private const nameRepertoireImage = 'pr_port/t_pr_01_infrastructure/';
    ...
    public function __construct(ParameterBagInterface $params, KernelInterface  $kernelInterface) {
        ...
        $this->pathImagePort = $params->get('pathImagePort');
        $this->pathPublic = $params->get('pathPublic');
        $this->pathForNamePort = $params->get('pathForNamePort');
    ...
    /**
     * @Route("/api/port/getphoto/{id}", name="infra_port_photo", methods={"GET"})
     */
    public function getPhotosByInfra($id, Request $request, PortService $portService)
    {
        $infoPhotosInfra = [];
        $response = new Response();
        if (isset($id) && !empty($id)) {
            $infoPhotosInfra = $portService->getPhotoInfraInfo($id);
    ...
    /**
     * @Route("/api/port/deletephoto", name="port_delete_photo", methods={"POST"})
     */
    public function deletePhoto(Request $request, PortService $portService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                $infoPhotosInfra = $portService->getPhotoInfraInfo($idInfra);
                ...
                $directory = $this->pathImagePort . $indexPhoto."/";
                ...
                if (array_key_exists($indexPhoto, $oldPhotosInfra)) {
                    ...
                    if (isset($setUpdate) && !empty($setUpdate)) {
                        $idInfra = $portService->addInfrastructurePhoto($idInfra, $setUpdate);
                    }
                   
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Photo port route deleted_successfull"
                    ]));
                } else {
                    $response->setContent(json_encode([
                        'code'  => Response::HTTP_OK,
                        'status' => true,
                        'message' => "Pas de photo port route supprimer"
                    ]));
                }
    ...
    /**
     * @Route("/api/port/updatephoto", name="port_update_photo", methods={"POST"})
     */
    public function updatePhoto(Request $request, PortService $portService)
    { 
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if(!is_dir($this->pathImagePort)) {
                mkdir($this->pathImagePort, 0777, true);
            }
          
            $directory1 = $this->pathImagePort . "photo1/";
            ...
            if (null != $uploadedFile1 && "null" != $uploadedFile1 && "undefined" != $uploadedFile1) {
                ...
                $data['photo1'] = $this->pathForNamePort."photo1/" .$nomPhoto1;
            ...
            $directory2 = $this->pathImagePort . "photo2/";
            ...
            if (null != $uploadedFile2 && "null" != $uploadedFile2 && "undefined" != $uploadedFile2) {
                ...
                $data['photo2'] = $this->pathForNamePort."photo2/" .$nomPhoto2;
            ...
            $directory3 = $this->pathImagePort . "photo3/";
            ...
            if (null != $uploadedFile3 && "null" != $uploadedFile3 && "undefined" != $uploadedFile3) {
                ...
                $data['photo3'] = $this->pathForNamePort."photo3/" .$nomPhoto3;
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "Photo port route updated_successfull"
            ]));
    ...
    /**
     * @Route("/api/port/add", name="port_add", methods={"POST"})
     */
    public function create(Request $request, PortService $portService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if ($request->get('categorie') != "null" && $request->get('categorie') != "undefined") {
                $allCategories = $portService->getAllCategorieInfra();
            ...
            $data['existenceQuai'] = $request->get('existenceQuai');
            $data['existenceFissureQuai'] = $request->get('existenceFissureQuai');
            $data['existenceFerraillageVisibleQuai'] = $request->get('existenceFerraillageVisibleQuai');
            $data['existenceRade'] = $request->get('existenceRade');
            $data['existenceFissureRade'] = $request->get('existenceFissureRade');
            $data['existenceFerraillageVisibleRade'] = $request->get('existenceFerraillageVisibleRade');
            $data['existenceDarse'] = $request->get('existenceDarse');
            $data['existenceFerraillageVisibleDarse'] = $request->get('existenceFerraillageVisibleDarse');
            ...
            $idInfra = $portService->addInfrastructure($data);

            if ($idInfra != false) {
                // add situation et etat
                //$idEtat = $portService->addInfrastructureRouteEtat($idInfra, $data);

                $idEtat = $portService->addInfrastructureSituation($idInfra, $data);

                $idDataCollected = $portService->addInfrastructureDonneCollecte($idInfra, $data);

                /*$idStructure = $portService->addInfrastructureRouteStructure($idInfra, $data);

                $idAccotement = $portService->addInfrastructureRouteAccotement($idInfra, $data);

                $idFosse = $portService->addInfrastructureRouteFosse($idInfra, $data);*/
                ...
                $idFoncier = $portService->addInfrastructureRouteFoncier($idInfra, $data);*/
                ...
                if (null != $request->get('hasTravaux') && ($request->get('hasTravaux') == true || $request->get('hasTravaux') == "true") && "false" != $request->get('hasTravaux')) {
                    ...
                    $idTravaux = $portService->addInfrastructureTravaux($idInfra, $data);
                }
                ...
                $idFourniture = $portService->addInfrastructureRouteFourniture($idInfra, $data);*/
                // Etudes
                ...
                // $data['precisionConsistanceContratEtude'] = $request->get('precisionConsistanceContratEtude');
                    ...
                    $idEtude = $portService->addInfrastructureEtudes($idInfra, $data);
                }
                
                /**
                 * End Administrative data
                */
                //$idDonneAnnexe = $portService->addInfrastructureEducationDonneAnnexe($idInfra, $data);
            }

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "port route created_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            $portService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            $portService->cleanTablesByIdInfrastructure($idInfra, 'situation');
            $portService->cleanTablesByIdInfrastructure($idInfra, 'data');
            $portService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            $portService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
            $portService->cleanTablesByIdInfrastructure($idInfra, 'surface');
            $portService->cleanTablesByIdInfrastructure($idInfra, 'structure');
            
            $portService->cleanTablesByIdInfrastructure($idInfra, 'accotement');
            $portService->cleanTablesByIdInfrastructure($idInfra, 'fosse');
            $portService->cleanTablesByIdInfrastructure($idInfra, 'foncier');
           
            $portService->cleanTablesByIdInfrastructure($idInfra, 'fourniture');*/
    ...
    /**
     * @Route("/api/infra/port/liste", name="port_list", methods={"GET"})
     */
    public function listeport(Request $request, PortService $portService)
    {    
        $response = new Response();
        
        try {

            $routes = $portService->getAllInfrastructures();

            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "port route list_successfull",
                'pathImage' => $this->pathImage,
                'data' => $routes
            ]));
    ...
    /**
     * @Route("/api/infra/port/liste/minifie", name="port_list_minifie", methods={"GET"})
     */
    public function listeportMinifie(Request $request, PortService $portService)
    {    
        $response = new Response();
        
        try {

            $routes = $portService->getAllInfrastructuresMinifie();
    ...
    /**
     * @Route("/api/infra/port/info", name="port_info", methods={"POST"})
     */
    public function getOneInfraInfo(Request $request, PortService $portService)
    {    
        $response = new Response();
        ...
        try {
            ...
            $routes = $portService->getOneInfraInfo(intval($infraId));
    ...
    /**
     * @Route("/api/port/update", name="port_update", methods={"POST"})
     */
    public function update(Request $request, PortService $portService)
    {    
        $response = new Response();
        $hasException = false;
        $idInfra = null;
        try {
            ...
            if (0 === strpos($request->headers->get('Content-Type'), 'application/json')) {
                ...
                if (array_key_exists('infrastructure', $data) && count($data['infrastructure']) > 0) {
                    ...
                    $idInfra = $portService->updateInfrastructure($idInfra, $updateColonneInfra);
                    }
                }
                // Situation
                ...
                if (array_key_exists('situations', $data) && count($data['situations']) > 0) {
                    ...
                    if ($idSituation == 0) {
                        $idSituation = $portService->addInfoInTableByInfrastructure('t_pr_03_situation', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtat) && !empty($updateColonneEtat)) {
                        $idSituation = $portService->updateInfrastructureTables('t_pr_03_situation', $idSituation, $updateColonneEtat);
                        }
                    } 
                ...
                if (array_key_exists('data_collecte', $data) && count($data['data_collecte']) > 0) {
                    ...
                    if ($idData == 0) {
                        $idData = $portService->addInfoInTableByInfrastructure('t_pr_06_donnees_collectees', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneData) && !empty($updateColonneData)) {
                        $idData = $portService->updateInfrastructureTables('t_pr_06_donnees_collectees', $idData, $updateColonneData);
                        }
                    }
                }
                // Travaux
                ...
                if (array_key_exists('travaux', $data) && count($data['travaux']) > 0) {
                    ...
                    if ($idTravaux == 0) {
                        $idTravaux = $portService->addInfoInTableByInfrastructure('t_pr_08_travaux', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneTravaux) && !empty($updateColonneTravaux)) {
                        $idTravaux = $portService->updateInfrastructureTables('t_pr_08_travaux', $idTravaux, $updateColonneTravaux);
                        }
                    }
                }
                ...
                if (array_key_exists('etudes', $data) && count($data['etudes']) > 0) {
                    ...
                    if ($idEtudes == 0) {
                        $idEtudes = $portService->addInfoInTableByInfrastructure('t_pr_10_etudes', $colonneInsert, $valuesInsert);
                    } else {
                        if (isset($updateColonneEtudes) && !empty($updateColonneEtudes)) {
                        $idEtudes = $portService->updateInfrastructureTables('t_pr_10_etudes', $idEtudes, $updateColonneEtudes);
                        }
                    }
                }
            }
            ...
            $response->setContent(json_encode([
                'code'  => Response::HTTP_OK,
                'status' => true,
                'message' => "port update_successfull"
            ]));
        ...
        if ($hasException) {// Clean database
            //$portService->cleanTablesByIdInfrastructure($idInfra, 'infrastructure');
            //$portService->cleanTablesByIdInfrastructure($idInfra, 'etat');
            //$portService->cleanTablesByIdInfrastructure($idInfra, 'data');
            //$portService->cleanTablesByIdInfrastructure($idInfra, 'travaux');
            //$portService->cleanTablesByIdInfrastructure($idInfra, 'etude');
            /*
