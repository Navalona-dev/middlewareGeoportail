...
class DalotRepository extends ServiceEntityRepository
{
    ...
    {
        $sql = "SELECT infra.id, infra.nom, infra.localite, infra.commune_terrain, infra.pk_implantation, infra.date_information, infra.source_information as source_info_infra, infra.type, infra.district, infra.materiaux_radier, infra.precision_materiaux_radier, infra.materiaux_piedroit, infra.materiaux_dalle, infra.precision_materiaux_dalle, infra.precision_materiaux_piedroit, infra.region,  ST_X(infra.geom) AS long, ST_Y(infra.geom) AS lat, infra.photo_name1, infra.photo_name2, infra.photo_name3, etat.id as etat_id, etat.etat as etat_etat, etat.fonctionnel as etat_fonctionnel, etat.raison as etat_raison, etat.date_information as etat_date_information, etat.source_information as etat_source_information, etat.mode_acquisition_information as etat_mode_acquisition_information, dc.id as data_id, dc.existence_de_fissures as data_existence_de_fissures, dc.niveau_ensablement_de_l_ouverture as data_niveau_ensablement_de_l_ouverture, dc.date_information as data_date_information, dc.source_information as data_source_information, dc.mode_acquisition_information as data_mode_acquisition_information, dc.autre_degradation as data_autre_degradation, trav.id as travaux_id, trav.bailleur as travaux_bailleur, trav.objet as travaux_objet, trav.consistance_travaux as travaux_consistance_travaux, trav.maitre_ouvrage as travaux_maitre_ouvrage, trav.maitre_ouvrage_delegue as travaux_maitre_ouvrage_delegue, trav.maitre_oeuvre as travaux_maitre_oeuvre, trav.id_controle_surveillance as travaux_id_controle_surveillance, trav.mode_passation as travaux_mode_passation, trav.porte_appel_offre as travaux_porte_appel_offre, trav.montant as travaux_montant, trav.numero_contrat as travaux_numero_contrat, trav.date_contrat as travaux_date_contrat, trav.date_ordre_service as travaux_date_ordre_service, trav.id_titulaire as travaux_id_titulaire, trav.resultat_travaux as travaux_resultat_travaux, trav.motif_rupture_contrat as travaux_motif_rupture_contrat, trav.date_reception_provisoire as travaux_date_reception_provisoire, trav.date_reception_definitive as travaux_date_reception_definitive, trav.ingenieur_reception_provisoire as travaux_ingenieur_reception_provisoire, trav.ingenieur_reception_definitive as travaux_ingenieur_reception_definitive, trav.date_information as travaux_date_information, trav.source_information as travaux_source_information, trav.mode_acquisition_information as travaux_mode_acquisition_information, etude.id as etude_id, etude.consistance_contrat as etude_consistance_contrat, etude.bailleur as etude_bailleur, etude.objet_contrat as etude_objet_contrat, etude.entite as etude_entite, etude.id_titulaire as etude_id_titulaire, etude.montant_contrat as etude_montant_contrat, etude.numero_contrat as etude_numero_contrat, etude.mode_passation as etude_mode_passation, etude.porte_appel_offre as etude_porte_appel_offre, etude.date_contrat as etude_date_contrat, etude.date_ordre_service as etude_date_ordre_service, etude.resultat_prestation as etude_resultat_prestation, etude.motif_rupture_contrat as etude_motif_rupture_contrat, etude.date_information as etude_date_information, etude.source_information as etude_source_information, etude.mode_acquisition_information as etude_mode_acquisition_information  FROM t_dar_01_infrastructure as infra LEFT JOIN t_dar_03_etat as etat ON infra.id = etat.id_infrastructure  LEFT JOIN t_dar_04_donnees_collectees as dc ON infra.id = dc.id_infrastructure  LEFT JOIN t_dar_05_travaux as trav ON infra.id = trav.id_infrastructure LEFT JOIN t_dar_07_etudes as etude ON infra.id = etude.id_infrastructure where infra.id = ".$infraId."";
    ...
    {
        $sql = 'SELECT infra.id as infra_id, infra.nom as nom, infra.localite, infra.commune_terrain, infra.pk_implantation, infra.date_information,  ST_X(infra.geom) AS long, ST_Y(infra.geom) AS lat, infra.photo_name1, infra.photo_name2, infra.photo_name3  FROM t_dar_01_infrastructure as infra';
