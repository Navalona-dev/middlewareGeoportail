...
class DalotService
{
    ...
    {
        $result = $this->dalotRepository->updateInfrastructure($idInfra, $updateColonneInfra);
        return $result;
    }
    ...
    {
        $routes = $this->dalotRepository->getAllInfrastructuresMinifie();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $result = $this->dalotRepository->addInfrastructurePhoto($idInfrastructure, $setUpdate);
    ...
    {
        $routes = $this->dalotRepository->getPhotoInfraInfo(intval($infraId));
        if (count($routes) > 0) {
            return $routes;
        }
        return false;
