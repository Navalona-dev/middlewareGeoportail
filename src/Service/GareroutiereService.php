...
use App\Repository;
use App\Repository\GareroutiereRepository;

class GareroutiereService
{
    private $tokenStorage;
    private $entityManager;
    private $session;
    private $gareroutiereRepository;

    public function __construct(TokenStorageInterface  $TokenStorageInterface, EntityManager $entityManager, SessionInterface $session, GareroutiereRepository $gareroutiereRepository)
    {
    ...
    {
        $result = $this->gareroutiereRepository->addInfrastructure($data['nom'], $data['categorie'], $data['localite'], $data['communeTerrain'], $data['capaciteVoitureAccueillies'], $data['sourceInformation'], $data['modeAcquisitionInformation'], $data['longitude'], $data['latitude'], $data['district'], $data['categoriePrecision'], $data['code'], $data['region'], $data['photo1'], $data['photo2'], $data['photo3'], $data['photoName1'], $data['photoName2'], $data['photoName3']);
        return $result;
    }

    public function addInfrastructureBaseRoute($multipleCoordonnée, $nom)
    {
        $result = $this->gareroutiereRepository->addInfrastructureBaseRoute($multipleCoordonnée, $nom);
        return $result;
    }

    public function getAllInfrastructures()
    {
        $routes = $this->gareroutiereRepository->getAllInfrastructures();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $result = $this->gareroutiereRepository->addInfrastructureRouteEtat($idInfrastructure, $data['etat'], $data['sourceInformation'], $data['modeAcquisitionInformation']);
    ...
    {
        $result = $this->gareroutiereRepository->addInfrastructureSituation($idInfrastructure, $data['fonctionnel'], $data['motif'], $data['sourceInformationSituation'], $data['modeAcquisitionInformationSituation'], $data['etat']);
    ...
    {
        $result = $this->gareroutiereRepository->addInfrastructureDonneCollecte($idInfrastructure, $data['revetementParking'], $data['etatParking'], $data['etatGlobalAccessoires'], $data['sourceInformationData'], $data['modeAcquisitionInformationData']);
    ...
    {
        $result = $this->gareroutiereRepository->addInfrastructureRouteFoncier($idInfrastructure, $data['statut'], $data['numeroReference'], $data['nomProprietaire'], $data['sourceInformationFoncier'], $data['modeAcquisitionInformationFoncier']);
    ...
    {
        $this->gareroutiereRepository->cleanTablesByIdInfrastructure($idInfrastructure, $type);
    }

    public function getAllyRouteInfo()
    {
        $routeyInfo = $this->gareroutiereRepository->getAllyRouteInfo();
