...
use App\Repository;
use App\Repository\InfrastructureaviationRepository;

class InfrastructureaviationService
{
    private $tokenStorage;
    private $entityManager;
    private $session;
    private $infrastructureaviationRepository;

    public function __construct(TokenStorageInterface  $TokenStorageInterface, EntityManager $entityManager, SessionInterface $session, InfrastructureaviationRepository $infrastructureaviationRepository)
    {
        $this->tokenStorage = $TokenStorageInterface;
        $this->entityManager = $entityManager;
        $this->session = $session;
        $this->infrastructureaviationRepository = $infrastructureaviationRepository;
    }
    
    public function addInfrastructure($data)
    {
        $result = $this->infrastructureaviationRepository->addInfrastructure($data['nom'], $data['categorie'], $data['indicatif'], $data['longueurPiste'], $data['largeurPiste'], $data['localite'], $data['communeTerrain'], $data['sourceInformation'], $data['modeAcquisitionInformation'], $data['longitude'], $data['latitude'], $data['district'], $data['categoriePrecision'], $data['chargeMaximum'], $data['region'], $data['photo1'], $data['photo2'], $data['photo3'], $data['photoName1'], $data['photoName2'], $data['photoName3'], $data['moisOuverture'], $data['moisFermeture']);
        return $result;
    }

    public function getAllCategorieInfra()
    {
        $categories = $this->infrastructureaviationRepository->getAllCategorieInfra();
    ...
    {
        $routes = $this->infrastructureaviationRepository->getAllInfrastructures();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $routes = $this->infrastructureaviationRepository->getAllInfrastructuresMinifie();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $result = $this->infrastructureaviationRepository->updateInfrastructure($idInfra, $updateColonneInfra);
        return $result;
    }

    public function addInfoInTableByInfrastructure($table, $colonnes, $values)
    {
        $result = $this->infrastructureaviationRepository->addInfoInTableByInfrastructure($table, $colonnes, $values);
        return $result;
    }

    public function updateInfrastructureTables($table , $idRow, $updateColonne)
    {
        $result = $this->infrastructureaviationRepository->updateInfrastructureTables($table, $idRow, $updateColonne);
        return $result;
    }
    
    public function getOneInfraInfo($infraId)
    {
        $routes = $this->infrastructureaviationRepository->getOneInfraInfo(intval($infraId));
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $routes = $this->infrastructureaviationRepository->getAllInfrastructuresBaseRoute();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $result = $this->infrastructureaviationRepository->addInfrastructureRouteEtat($idInfrastructure, $data['etat'], $data['sourceInformation'], $data['modeAcquisitionInformation']);
    ...
    {
        $result = $this->infrastructureaviationRepository->addInfrastructureSituation($idInfrastructure, $data['fonctionnel'], $data['motif'], $data['sourceInformationSituation'], $data['modeAcquisitionInformationSituation'], $data['etat'], $data['raisonPrecision']);
    ...
    {
        $result = $this->infrastructureaviationRepository->addInfrastructureRouteFoncier($data['statut'], $data['numeroReference'], $data['nomProprietaire'], $idInfrastructure);
    ...
    {
        $result = $this->infrastructureaviationRepository->addInfrastructurePhoto($idInfrastructure, $setUpdate);
    ...
    {
        $routes = $this->infrastructureaviationRepository->getPhotoInfraInfo(intval($infraId));
        if (count($routes) > 0) {
            return $routes;
        }
        return false;
    ...
    {
        $this->infrastructureaviationRepository->cleanTablesByIdInfrastructure($idInfrastructure, $type);
    }

    public function getAllyRouteInfo()
    {
        $routeyInfo = $this->infrastructureaviationRepository->getAllyRouteInfo();
