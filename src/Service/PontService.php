...
class PontService
{
    ...
    private $session;
    private $pontRepository;

    public function __construct(TokenStorageInterface  $TokenStorageInterface, EntityManager $entityManager, SessionInterface $session, PontRepository $pontRepository)
    {
    ...
    {
        $result = $this->pontRepository->addInfrastructure($data['nom'], $data['categorie'], $data['typePont'], $data['nomRouteRattache'], $data['pointKmImplantation'], $data['localite'], $data['communeTerrain'], $data['sourceInformation'], $data['modeAcquisitionInformation'], $data['longitude'], $data['latitude'], $data['district'], $data['categoriePrecision'],$data['typePrecision'], $data['longueur'], $data['largeur'], $data['nombreVoies'], $data['chargeMaximum'], $data['region'], $data['photo1'], $data['photo2'], $data['photo3'], $data['photoName1'], $data['photoName2'], $data['photoName3']);
        return $result;
    }

    public function addInfrastructureBaseRoute($multipleCoordonnée, $nom)
    {
        $result = $this->pontRepository->addInfrastructureBaseRoute($multipleCoordonnée, $nom);
        return $result;
    }
    ...
    {
        $result = $this->pontRepository->addInfrastructureRouteEtat($idInfrastructure, $data['etat'], $data['sourceInformation'], $data['modeAcquisitionInformation']);
    ...
    {
        $result = $this->pontRepository->addInfrastructureSituation($idInfrastructure, $data['fonctionnel'], $data['motif'], $data['sourceInformationSituation'], $data['modeAcquisitionInformationSituation'], $data['etat'], $data['raisonPrecision']);
    ...
    {
        $result = $this->pontRepository->addInfrastructureDonneCollecte($idInfrastructure, $data['typePile'], $data['pileBetonExistenceFissure'], $data['pileBetonExistenceFerraillageVisible'], $data['pilesMetalliquesCompletude'], $data['pilesMetalliquesExistenceRouille'], $data['existenceGardeCorps'], $data['typeGardeCorps'], $data['gardeCorpsMetalliqueCompletude'], $data['gardeCorpsMetalliqueExistenceRouille'], $data['gardeCorpsBetonExistenceFissure'], $data['gardeCorpsBetonExistenceFerraillageVisible'], $data['decalageJointureTablierChausseeAffaissement'], $data['decalageJointureTablierChausseeEcartement'], $data['typeTablier'], $data['tablierBetonExistenceFissure'], $data['tablierBetonExistenceFerraillageVisible'], $data['tablierMetalliqueCompletude'], $data['tablierMetalliqueExistenceRouille'], $data['typePoutre'], $data['poutreBetonExistenceFissure'], $data['poutreBetonExistenceFerraillageVisible'], $data['poutreMetalliqueCompletude'], $data['poutreMetalliqueExistenceRouille'], $data['culesExistenceAffouillement'], $data['culesExistenceFissure'], $data['culesExistenceFerraillageVisible'], $data['sourceInformationData'], $data['modeAcquisitionInformationData'], $data['tablierBoisGonflement'], $data['tablierBoisFissure'], $data['tablierBoisAgents'], $data['tablierBoisDeformation'], $data['poutreBoisDeformation'], $data['poutreBoisFissure'], $data['poutreBoisAgents'], $data['poutreBoisGonflement']);
    ...
    {
        $result = $this->pontRepository->addInfrastructureRouteAccotement($idInfrastructure, $data['accotement'], $data['accotementDegrationSurface'], $data['accotementDentelleRive'], $data['accotementDenivellationChausseAccotement'], $data['accotementDestructionAffouillementAccotement'], $data['accotementNonRevetueDeformationProfil'], $data['accotementRevetue'], $data['accotementTypeRevetementAccotement'], $data['accotementPrecisionTypeAccotement'], $data['dateInformationAccotement'], $data['sourceInformationAccotement'], $data['modeAcquisitionInformationAccotement']);
    ...
    {
        $result = $this->pontRepository->addInfrastructureRouteFosse($idInfrastructure, $data['coteFosse'], $data['fosseRevetuDegradationFosse'], $data['fosseRevetuSectionBouche'], $data['fosseNonRevetuFosseProfil'], $data['fosseNonRevetuEncombrement'], $data['fosseRevetu'], $data['dateInformationFosse'], $data['sourceInformationFosse'], $data['modeAcquisitionInformationFosse']);
    ...
    {
        $result = $this->pontRepository->addInfrastructureRouteFoncier($data['statut'], $data['numeroReference'], $data['nomProprietaire'], $idInfrastructure);
    ...
    {
        $routes = $this->pontRepository->getPhotoInfraInfo(intval($infraId));
        if (count($routes) > 0) {
            return $routes;
        }
        return false;
    ...
    {
        $this->pontRepository->cleanTablesByIdInfrastructure($idInfrastructure, $type);
    }

    public function getAllyRouteInfo()
    {
        $routeyInfo = $this->pontRepository->getAllyRouteInfo();
