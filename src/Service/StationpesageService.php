...
use App\Repository;
use App\Repository\StationpesageRepository;

class StationpesageService
{
    private $tokenStorage;
    private $entityManager;
    private $session;
    private $stationpesageRepository;

    public function __construct(TokenStorageInterface  $TokenStorageInterface, EntityManager $entityManager, SessionInterface $session, StationpesageRepository $stationpesageRepository)
    {
        $this->tokenStorage = $TokenStorageInterface;
        $this->entityManager = $entityManager;
        $this->session = $session;
        $this->stationpesageRepository = $stationpesageRepository;
    }
    
    public function addInfrastructure($data)
    {
        $result = $this->stationpesageRepository->addInfrastructure($data['nom'], $data['categorie'], $data['indicatif'], $data['nomRouteRattache'], $data['pointKmImplantation'], $data['localite'], $data['communeTerrain'], $data['sourceInformation'], $data['modeAcquisitionInformation'], $data['longitude'], $data['latitude'], $data['district'], $data['region'], $data['photo1'], $data['photo2'], $data['photo3'], $data['photoName1'], $data['photoName2'], $data['photoName3']);
        return $result;
    }

    public function addInfrastructureBaseRoute($multipleCoordonnée, $nom)
    {
        $result = $this->stationpesageRepository->addInfrastructureBaseRoute($multipleCoordonnée, $nom);
        return $result;
    }

    public function getAllInfrastructures()
    {
        $routes = $this->stationpesageRepository->getAllInfrastructures();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $routes = $this->stationpesageRepository->getAllInfrastructuresMinifie();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $result = $this->stationpesageRepository->updateInfrastructure($idInfra, $updateColonneInfra);
        return $result;
    }
    ...
    {
        $result = $this->stationpesageRepository->updateInfrastructureTables($table, $idRow, $updateColonne);
        return $result;
    }
    
    public function getOneInfraInfo($infraId)
    {
        $routes = $this->stationpesageRepository->getOneInfraInfo(intval($infraId));
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $routes = $this->stationpesageRepository->getAllInfrastructuresBaseRoute();
        if (count($routes) > 0) {
            return $routes;
        }
        return 0;
    ...
    {
        $result = $this->stationpesageRepository->addInfrastructureRouteEtat($idInfrastructure, $data['etat'], $data['sourceInformation'], $data['modeAcquisitionInformation']);
    ...
    {
        $result = $this->stationpesageRepository->addInfrastructureSituation($idInfrastructure, $data['fonctionnel'], $data['motif'], $data['sourceInformationSituation'], $data['modeAcquisitionInformationSituation'], $data['etat']);
    ...
    {
        $result = $this->stationpesageRepository->addInfrastructureDonneCollecte($idInfrastructure, $data['etatPesage'], $data['sourceInformationData'], $data['modeAcquisitionInformationData']);
    ...
    {
        $result = $this->stationpesageRepository->addInfrastructureRouteAccotement($idInfrastructure, $data['accotement'], $data['accotementDegrationSurface'], $data['accotementDentelleRive'], $data['accotementDenivellationChausseAccotement'], $data['accotementDestructionAffouillementAccotement'], $data['accotementNonRevetueDeformationProfil'], $data['accotementRevetue'], $data['accotementTypeRevetementAccotement'], $data['accotementPrecisionTypeAccotement'], $data['dateInformationAccotement'], $data['sourceInformationAccotement'], $data['modeAcquisitionInformationAccotement']);
    ...
    {
        $result = $this->stationpesageRepository->addInfrastructureRouteFosse($idInfrastructure, $data['coteFosse'], $data['fosseRevetuDegradationFosse'], $data['fosseRevetuSectionBouche'], $data['fosseNonRevetuFosseProfil'], $data['fosseNonRevetuEncombrement'], $data['fosseRevetu'], $data['dateInformationFosse'], $data['sourceInformationFosse'], $data['modeAcquisitionInformationFosse']);
    ...
    {
        $result = $this->stationpesageRepository->addInfrastructureRouteFoncier($data['statut'], $data['numeroReference'], $data['nomProprietaire'], $idInfrastructure);
    ...
    {
        $result = $this->stationpesageRepository->addInfrastructurePhoto($idInfrastructure, $setUpdate);
    ...
    {
        $routes = $this->stationpesageRepository->getPhotoInfraInfo(intval($infraId));
        if (count($routes) > 0) {
            return $routes;
        }
        return false;
    ...
    {
        $this->stationpesageRepository->cleanTablesByIdInfrastructure($idInfrastructure, $type);
    }

    public function getAllyRouteInfo()
    {
        $routeyInfo = $this->stationpesageRepository->getAllyRouteInfo();
        if (count($routeyInfo) > 0) {
            return $routeyInfo;
        }
        return false;
